...
public final class PagesOfAtlasPager {
    ...
    public interface Entry { int width(); int height(); String name(); }
    private static int powerOfTwo(int value) { return value <= 1 ? 1 : Integer.highestOneBit(value - 1) << 1; }

    public static <T extends Entry> Result<T> pack(
        List<T> input,
        int maxWidth,
        int maxHeight,
        int mipLevel,
     ...
     */
    public static <T extends Entry> Result<T>
    ...
    private static <T extends Entry> Result<T> pack(
        List<T> input,
        int maxWidth,
        int maxHeight,
        int mipLevel,
        ...
        if (maxWidth < 1 || maxHeight < 1 || mipLevel < 0 || mipLevel > 20 || padding < 0) {
            throw new IllegalArgumentException("Invalid page limits, mip level, or padding");
        }
        java.util.Set<String> names = new java.util.HashSet<>();
        for (T entry : input) {
            if (entry.width() < 1 || entry.height() < 1 || entry.width() > maxWidth || entry.height() > maxHeight || !names.add(entry.name())) {
                throw new IllegalArgumentException("Invalid, oversized, or duplicate sprite: " + entry.name());
            }
        }
    ...
    private static <T extends Entry> Holder<T>
        createHolder(
            T entry,
            int mipLevel,
            int padding
    ...
    private static final class Holder<T extends Entry> {
        final T entry;
        final int width;
        final int height;
    ...
    public static final class Page<T extends Entry> {
        ...
        private Region<T> expand(Holder<T> holder) {
            int previousX = storageX;
            int previousY = storageY;
            int xCurrentSize =
                powerOfTwo(storageX);
    ...
    private static final class Region<T extends Entry> {
        final int originX;
        final int originY;
        final int width;
        final int height;
    ...
    public record Placement<T extends Entry>(
        int page,
        T entry,
        int x,
        int y,
    ...
    public record Result<T extends Entry>(
        List<Page<T>> pages
    ) {}
}
